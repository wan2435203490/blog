  ...
  <div v-if="!$common.isEmpty(article)">
    ...
    <div class="article-head my-animation-slide-top">
      ...
      <el-image class="article-image my-el-image"
                v-once
                lazy
                :src="!$common.isEmpty(article.articleCover)?article.articleCover:$constant.random_image"
                fit="cover">
      ...
      <div class="article-info-news"
           ...
           v-if="!$common.isEmpty($store.state.user.current) && $store.state.user.current.id === article.userId">
...
<script>
  import { saveNews, listNews, deleteWeiYan} from "@/api/wei_yan";
  ...
  export default {
    ...
    methods: {
      ...
      getArticle() {
        getArticleById({id: this.id, viewStatus: true})
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
