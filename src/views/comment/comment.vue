...
<!--              <span v-if="item.childComments.total > 0">{{item.childComments.total}} </span><span>回复</span>-->
              <span v-if="!$common.isEmpty(item.childComments)">{{item.childComments.total}} </span><span>回复</span>
            </div>
          </div>
          <!-- 评论内容 -->
              ...
              <div style="flex: 1;padding-left: 12px">
                ...
                <div style="display: flex;justify-content: space-between">
                  <div>
                    ...
                    <span class="commentInfo-other">{{ $common.getDateDiff(childItem.createdAt) }}</span>
                  </div>
                  <div>
...
<script>
  import { getCommentCount, listComment, saveComment } from "@/api/comment";
  ...
  export default {
    ...
    methods: {
      ...
      getTotal() {
        getCommentCount({source: this.source, type: this.type})
          .then((res) => {
