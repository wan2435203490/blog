  ...
  export default {
    ...
    methods: {
      ...
      beforeUpload(file) {
        ...
        this.qiniuParam.key = this.prefix + "/" + (!this.$common.isEmpty(this.$store.state.user.current.username) ? (this.$store.state.user.current.username.replace(/[^a-zA-Z]/g, '') + this.$store.state.user.current.id) : (this.$store.state.user.current.username.replace(/[^a-zA-Z]/g, '') + this.$store.state.user.current.id)) + new Date().getTime() + Math.floor(Math.random() * 1000) + suffix;
      },
