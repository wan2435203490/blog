  ...
  <div>
    ...
    <div class="bg-wrap my-animation-slide-top">
      ...
      <div class="love-wrap transformCenter">
        ...
        <div>
           <span class="svg-container love-img">
            <svg-icon icon-class="biglove" class-name="love-svg"/>
           </span>
          ...
          <div class="family-container" v-show="!$common.isEmpty(randomFamily)">
            <div v-for="(item, index) in randomFamily" :key="index"
              ...
              <div class="family-wrap"
                ...
                <div>
                   <span class="family-svg-container family-img">
                    <svg-icon icon-class="like" class-name="family-svg"/>
                  </span>
...
<script>
  import {getAdminFamily, getFamily, listRandomFamily, saveFamily} from "@/api/family";
  import {deleteWeiYan, listWeiYan} from "@/api/wei_yan";
  import {listAdminLovePhoto, listResourcePath} from "@/api/web_info";
  ...
  export default {
    ...
    methods: {
      ...
      getRandomFamily() {
        listRandomFamily()
          .then((res) => {
            if (!this.$common.isEmpty(res.data.records)) {
              this.randomFamily = res.data.records;
            }
          })
          .catch((error) => {
            this.$message({
      ...
      changePhoto() {
        listResourcePath(this.photoPagination)
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
      ...
      getWeiYan() {
        listWeiYan(this.weiYanPagination)
          .then((res) => {
            if (!this.$common.isEmpty(res.data)) {
...
<style scoped>
  ...
  .love-img {
    animation: imgScale 2s ease-in-out infinite;
    width: 120px;
    height: 120px;
  }
