...
<script>
  ...
  import { saveResourcePath,listResourcePath,deleteResourcePath } from '@/api/web_info'
  ...
  export default {
    ...
    methods: {
      ...
      addResourcePath() {
        ...
        saveResourcePath(this.resourcePath, this.isUpdate)
          .then((res) => {
            this.$message({
              message: "保存成功！",
              type: "success"
      ...
      getResourcePaths() {
        if(this.pagination.status == null){
      ...
      handleDelete(item) {
        this.$confirm('确认删除？', '提示', {
          ...
          deleteResourcePath({ids: [item.id]})
            .then((res) => {
              this.search();
              this.$message({
                message: "删除成功！",
