...
<script>
  ...
  import { listSortAndLabel } from '@/api/web_info'
  import { getArticleById } from '@/api/admin'
  ...
  export default {
    ...
    methods: {
      getUpToken() {
        getUpToken(this.$constant.baseURL + "/qiniu/getUpToken", {}, true)
          .then((res) => {
      ...
      imgAdd(pos, file) {
        ...
        fd.append("key", "articlePicture" + "/" + this.$store.state.user.current.username.replace(/[^a-zA-Z]/g, '') + this.$store.state.user.current.id + new Date().getTime() + Math.floor(Math.random() * 1000) + suffix);

        this.$http.uploadQiniu(this.$constant.qiniuUrl, fd)
          .then((res) => {
            if (!this.$common.isEmpty(res.key)) {
              let url = this.$constant.qiniuDownload + res.key;
              this.$common.saveResource(this, "articlePicture", url, file.size, file.type);
      ...
      getSortAndLabel() {
        listSortAndLabel()
          .then((res) => {
            // console.log("listSortAndLabel.success", res)
            if (!this.$common.isEmpty(res.data)) {
