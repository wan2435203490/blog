...
export default {
  ...
  cbcKey(){
    return CryptoJS.enc.Utf8.parse(constant.cryptojs_key);
  },

  cbcIv(){
    return CryptoJS.enc.Utf8.parse(CryptoJS.SHA256(constant.cryptojs_key).toString());
  },
  //CBC加密
  encrypt(data) {
    var cfg = {
    ...
    let res = CryptoJS.AES.encrypt(data, this.cbcKey(), cfg).toString();
    return res;
  },
  ...
  decrypt(data) {
    ...
    var decrypt = CryptoJS.AES.decrypt(data, this.cbcKey(), cfg);
    // console.log("decrypt-utf8:",decrypt.toString(CryptoJS.enc.Utf8))
